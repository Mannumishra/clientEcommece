...
export default function Checkout() {
    ...
    const getAPIData = async () => {
        try {
            let res = await axios.get('https://induserver.assortsmachinetools.com/api/user/' + sessionStorage.getItem("userid"));
    ...
    const getCartData = async () => {
        try {
            let res = await axios.get('https://induserver.assortsmachinetools.com/api/cart/' + sessionStorage.getItem("userid"));
    ...
    const placeOrder = async () => {
        try {
            ...
            try {
                if (mode === "COD") {
                    const res = await axios.post('https://induserver.assortsmachinetools.com/api/checkout', item);
                    console.log(res)
                    if (res.status === 200) {
                        toast.success("Order Place Successfully")
                        for (let items of cart) {
                            let deleteItem = await axios.delete("https://induserver.assortsmachinetools.com/api/cart/" + items._id)
                    ...
                    if (res.data.success) {
                        ...
                        const options = {
                            ...
                            currency: "INR",
                            name: "Indu enterprises",
                            description: `Payment For Indu enterprises Product`,
                            image: "https://i.pinimg.com/originals/9e/ff/85/9eff85f9a3f9540bff61bbeffa0f6305.jpg",
                            order_id: order?.id,
                            callback_url: `https://induserver.assortsmachinetools.com/api/Payment-Verification`,
                            prefill: {
                                contact: user.phone
                            },
                            notes: {
