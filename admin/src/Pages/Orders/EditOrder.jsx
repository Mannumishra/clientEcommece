...
const EditOrder = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://induserver.assortsmachinetools.com/api/checkout/admin/${_id}`);
        console.log(response)
        setData(response.data.data);
        setOrderstatus(response.data.data.orderstatus);
        setPaymentstatus(response.data.data.paymentstatus);
        const userResponse = await axios.get(`https://induserver.assortsmachinetools.com/api/user/${response.data.data.userid}`);
  ...
  const updateItem = async () => {
    let res = await axios.put("https://induserver.assortsmachinetools.com/api/checkout/admin/" + _id, { ...data, orderstatus: orderstatus, paymentstatus: paymentstatus })
    if (res.status === 200) {
      navigate("/all-orders")
    }
  }
